...
import com.goodthings.app.util.DensityUtil;
...
public class CustomCircleProgressBar extends View {
    ...
    private String progressTextCus = "";
    private String progressExtraText="";     //圆环内文字
    private Rect rect;
    ...
    public CustomCircleProgressBar(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        outsideRadius = a.getDimension(R.styleable.CustomCircleProgressBar_outside_radius, DensityUtil.dp2px(getContext(), 60.0f));
        insideColor = a.getColor(R.styleable.CustomCircleProgressBar_inside_color, ContextCompat.getColor(getContext(), R.color.grey_white));
        ...
        progressTextSize = a.getDimension(R.styleable.CustomCircleProgressBar_progress_text_size, DensityUtil.dp2px(getContext(), 14.0f));
        progressWidth = a.getDimension(R.styleable.CustomCircleProgressBar_progress_width, DensityUtil.dp2px(getContext(), 10.0f));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if(progressTextCus.isEmpty()){
            progressText = getProgressText();
        }else{
            progressText = progressTextCus;
        }
        ...
        canvas.drawText(progressText, getMeasuredWidth() / 2 - rect.width() / 2, baseline-(rect.height()/2+5), paint);


        rect = new Rect();
        paint.setColor(Color.BLACK);
        paint.setTextSize(progressTextSize-10);
        paint.setStrokeWidth(0);
        paint.getTextBounds(progressExtraText, 0, progressExtraText.length(), rect);
        Paint.FontMetricsInt fontMetricse = paint.getFontMetricsInt();
        int baselinee = (getMeasuredHeight() - fontMetricse.bottom + fontMetricse.top) / 2 - fontMetricse.top;  //获得文字的基准线
        canvas.drawText(progressExtraText, getMeasuredWidth() / 2 - rect.width() / 2, baselinee+(rect.height()/2+5), paint);
    }
    ...
    public void setProgressExtraText(String text){
        this.progressExtraText = text;
    }

    public void setProgressTextCus(String textCus){
        this.progressTextCus = textCus;
    }
